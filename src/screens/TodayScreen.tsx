...
// Module-level cache — survives React remounts within the same session
type ProfileCache = {
  uid: string
  profile: UserProfile
...
export default function TodayScreen() {
  ...
  const cached = _profileCache?.uid === uid ? _profileCache : null
  ...
  const [dueDay, setDueDay] = useState<ProgramDay>(cached?.dueDay ?? PRESET_PROGRAMS[0].days[0])
  ...
  useEffect(() => {
    async function load() {
      ...
      const due = nextDayInProgram(p.lastWorkoutType, prog)

      _profileCache = { uid, profile: p, activeProgram: prog, dueDay: due }
      setProfile(p)
  ...
  return (
    <div className="bg-iron-950 flex flex-col overflow-hidden animate-screen-in" style={{ height: 'calc(100dvh - 5rem)' }}>
      ...
      <div className="flex flex-col flex-1 p-5 pt-8 gap-3 min-h-0">
        ...
        <div className="flex-1 flex flex-col items-center justify-center min-h-0">
          <p className="font-mono text-iron-500 text-[10px] tracking-widest uppercase mb-3">Due today</p>
          <div
            className="font-display leading-none text-center"
            style={{ fontSize: 'clamp(3.5rem, 20vw, 6.5rem)', color }}
          >
        ...
        {todayWorkout?.exists ? (
          <button
            ...
            className="flex-shrink-0 w-full py-4 font-sans font-bold uppercase text-sm text-black transition-opacity active:opacity-80"
